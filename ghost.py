...
class Ghoster:
    ...
    def game_loop(self):
        ...
        if random.random() > 0.5:
            ...
            players = [p2, p1]
        print('\n\nWelcome to ghost! To see the rules, open the readme.md included in this repository.\n' \
              f'{str(players[0]).capitalize()} will begin this round.\n')
        while not game_over:
            new_letter = self.get_input(players[0], current_str)
            ...
            if response_code == 0:
                ...
                players[0].strikes += 1
                print(f'\n\n"{current_str.capitalize()}{new_letter}" is not a valid beginning to any word.')
                if players[0].strikes == 3:
                    print(f'\n\nOh no, {players[0]}! That was your 3rd strike!\n' \
                          f'That means {players[0]} is the loser, and {players[1]} is the victor!')
                    game_over = True
                else:
                    strike_string = f'{str(players[0]).capitalize()}, you now have {players[0].strikes} strike'
                    if players[0].strikes == 1:
                        print(f'{strike_string}.')
                    else:
                        print(f'{strike_string}s.')
            elif response_code == 2:
                print(f'\n\nUh oh, {players[0]}! Since {current_str}{new_letter} is a real word, you lose!\n' \
    ...
    def get_input(self, player, current_str):
        while 1:
            if len(current_str) > 0:
                print(f'\nThe current string is {current_str}.')
